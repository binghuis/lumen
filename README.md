...
**技术可行性已端到端验通**:真实 B站 直播间跑通 `弹幕 → 豆包(人格+情绪)→ 火山语音 → Live2D 口型 → 导演层动作`。**技术命题成功 ✅,市场命题(有没有人看/打赏)待验。** 详见 [项目进度](docs/项目进度.md)。
...
| 形象 | 2D Live2D(VTube Studio,免费 Hiyori 占位)+ 音频口型 | ✅(商用要换授权模型) |
| 导演层 | 豆包 [情绪] → 触发 VTS 表情/动作 | ✅ 核心增量验通 |
